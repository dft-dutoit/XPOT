...
def prep_x_validation(input_directory, bins, include_test=True):
...
def convert_xyz_pickle(data_files, ref_e = 0, energy_key = "energy"):
    # Read in the data
    dataset = {
        "energy": [],
        "forces": [],
    ...
    if type(ref_e) == dict:
        ...
        for atoms in structures:
            ...
            for elem in elems:
                if elem in atoms.get_chemical_symbols():
                    total_ref_e += ref_e[elem]*atoms.get_chemical_symbols().count(elem)
            dataset["energy_corrected"].append(atoms.info[energy_key] - total_ref_e)
            total_ref_e = 0
        ...
        for atoms in structures:
            ...
            dataset["forces"].append(atoms.get_forces())
            dataset["ase_atoms"].append(atoms)
            dataset["energy_corrected"].append(atoms.info[energy_key] - (len(atoms) * ref_e))


    df = pd.DataFrame(dataset)
    df.to_pickle(f"{data_files}.pkl.gzip", compression="gzip")

def get_best_potential(directory, method="SNAP"):
    ...
    with open(f"{directory}/params.csv", "r") as f:
        ...
        shutil.copytree(f"{directory}/{best_iteration}", f"{directory}/best_potential_no{best_iteration}")
        if method == "GAP":
            pot_file = glob.glob(f"{directory}/best_potential_no{best_iteration}/*.xml")

        #elif method == "ACE":
        #    subprocess.run([
        #        "pace_yaml2yace",
        #        "-o",
        #        f"{directory}/best_potential_no{best_iteration}/*.yace",
        #        f"{directory}/best_potential_no{best_iteration}/*.yaml"
        #    ])
        #    pot_file = glob.glob(f"{directory}/best_potential_no{best_iteration}/*.yace")

        elif method == "SNAP":
            pot_files = glob.glob(f"{directory}/best_potential_no{best_iteration}/*.snap*")
            with open(f"{directory}/best_potential_no{best_iteration}/snap_input.in", "r") as f:
                ref_setup = []
            ...
            for entry in ref_setup:
                if entry[0] == "units" or entry[0] == "atom_style":
                    ...
                    pot_file["lmpcmds"] = [f"pair_style hybrid/overlay {entry[1]} snap"]
                    for i in [a for a in ref_setup if a[0].startswith("pair_coeff")] and i[1] != "* * zero":
                        pot_file["lmpcmds"].append(f"pair_coeff {i[1]}")
                    pot_file["lmpcmds"].append(f"pair_coeff * * snap {pot_files[0]} {pot_files[1]} ")
                                   
    return pot_file

def parse_previous_sweep(directory):
...
def potential_to_calculator(method, directory):
