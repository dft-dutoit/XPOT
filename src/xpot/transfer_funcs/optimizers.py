...
def sweep_init_gp(method, input_script=None, **kwargs):
    ...
    def init_csv(**data):
        ...
        with open(f"./{project}/{sweep}/atom_errors.csv", "a") as f:
            f.writelines(
                f"Iteration, {method} E/at. (train), {method} E/at. (test), {method} Force Components (training), {method} Force Components (testing),\n"
            )

    init_csv(opt_space)

    @skopt.utils.use_named_args(opt_space)
    def run_init(**params) -> float:
        nonlocal mlip_obj, n, sweep, data_dict
        #nonlocal n
        n += 1
        print(params)
        #try:
...
def sweep_init_gp_parallel(method, input_script=None, device="gpu", **kwargs):
    manager = Manager()
    ...
    def assign_device(device=device):
        if device=="cpu":
            ...
            print("Device not recognised, please choose 'cpu' or 'gpu'")
    ...
    def run_init(**params):
        ...
        print(params)
        process_id = str(Process.current_process().ident)
        ### THIS REGION REQUIRES FURTHER IMPROVEMENT ###
        if process_id not in gpu_assignments:
            assign_device(device="gpu")
    ...
    if device=="gpu":
        ...
        print("CPU parallelisation not yet implemented")
